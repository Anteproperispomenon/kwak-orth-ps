...
type InputFileSlot x = Hal.Slot InputFileQuery String x
...
inputFileComp :: forall m. (MonadAff m) => HC.Component InputFileQuery String String m
inputFileComp
  = Hal.mkComponent
    { initialState : \x -> {input : x, error : Nothing}
    , render : inputFileGUI
...
handleInputFileQuery :: forall a s m. Monad m => InputFileQuery a -> Hal.HalogenM InputFileState InputFileAction s String m (Maybe a)
handleInputFileQuery (InputString reply) = do
  stt <- get
  pure $ Just (reply stt.input)

handleInputFileAction :: forall m s. (MonadAff m) => InputFileAction -> Hal.HalogenM InputFileState InputFileAction s String m Unit 
...
handleInputFileAction (ChangeFile fl) = do
  ...
  case estr of
    ...
    Right str -> do
      Hal.put { input : str , error : Nothing}
      Hal.raise str
